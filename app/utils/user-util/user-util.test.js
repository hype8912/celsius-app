...
describe("isForPrimeTrustKYC()", () => {
  beforeEach(() => {
    ...
    nonPTStates = SELECT_VALUES.STATE.map(s => s.value).filter(
      s => !PRIMETRUST_KYC_STATES.includes(s)
    );
  });
...
describe("hasPassedKYC()", () => {
  ...
  it("should return false for other KYC statuses", () => {
    ...
    const notPassedStatuses = Object.values(KYC_STATUSES).filter(
      ks => ![icoPassed, passed].includes(ks)
    );
...
describe("isKYCRejectedForever()", () => {
  ...
  it("should return false for other KYC statuses", () => {
    ...
    const notPassedStatuses = Object.values(KYC_STATUSES).filter(
      ks => ![permanentlyRejected].includes(ks)
    );
    notPassedStatuses.forEach(ks => {
      mockState.user.profile.kyc.status = ks;
...
describe("mapProfile()", () => {
  ...
  it("should change profile picture url from http -> https", () => {
    ...
    expect(mapProfile(profile).profile_picture).toBe(
      "https://some.random.url/image.png"
    );
  });

...
export { simplexGetQuote, simplexCreatePaymentRequest };
...
function simplexGetQuote(coin, fiatCurrency, requestedCurrency, amount ) {
  return async dispatch => {
    try {
      ...
      dispatch(simplexCreatePaymentRequest(quoteId, coin, amount, fiatTotalAmount, fiatCurrency, fiatBaseAmount))
...
function simplexCreatePaymentRequest (quoteId, coin, amount, fiatTotalAmount, fiatCurrency, fiatBaseAmount) {
  return async dispatch => {
    try {
      dispatch(startApiCall(API.CREATE_PAYMENT_REQUEST));
      const paymentRequest = await simplexService.createPaymentRequest(quoteId, coin, amount, fiatTotalAmount, fiatCurrency, fiatBaseAmount);
